# Compute Packet Bit Parity — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement compute packet bit parity without string conversion.

## Exact rule

Return `popcount(x) % 2`: 0 for an even number of 1 bits and 1 for an odd number.

## Approach

Use masks and unsigned shifts to implement compute packet bit parity without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
