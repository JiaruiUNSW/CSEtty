# UTF-16 Code Unit Count — worked solution

## Idea in one sentence

Classify the scalar range first, then derive utf-16 code unit count from the UTF-8/UTF-16 boundary table.
...
Classify the scalar range first, then derive utf-16 code unit count from the UTF-8/UTF-16 boundary table. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
