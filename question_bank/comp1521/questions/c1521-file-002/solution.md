# POSIX Newline Count — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute posix newline count.

## Exact rule

Count bytes equal to hexadecimal `0A` (ASCII newline).

## Approach

Read the file as bytes and scan once to compute posix newline count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
