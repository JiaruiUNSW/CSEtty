# Recursive Tree Byte Total — worked solution

## Idea in one sentence

Use depth-first traversal and merge each entry into a summary for recursive tree byte total.

## Exact rule

Sum `st_size` for every real regular file below the root; an empty tree returns 0.

## Approach

Use depth-first traversal and merge each entry into a summary for recursive tree byte total. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
