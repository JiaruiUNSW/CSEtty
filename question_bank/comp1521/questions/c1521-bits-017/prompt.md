...
**Exact rule.** Let `s = k % 32`; take result bits `s..31` from `x` and bits `0..s-1` from `y` (so `s == 0` returns `x`).

Submit `c1521_bits_017.c`. Your program must not print prompts or explanatory text.
