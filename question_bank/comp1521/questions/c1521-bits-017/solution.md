# Merge Two Masked Device Words — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement merge two masked device words without string conversion.

## Exact rule

Let `s = k % 32`; take result bits `s..31` from `x` and bits `0..s-1` from `y` (so `s == 0` returns `x`).

## Approach

Use masks and unsigned shifts to implement merge two masked device words without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
