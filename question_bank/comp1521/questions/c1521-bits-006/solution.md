# Toggle Display Channels — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement toggle display channels without string conversion.

## Exact rule

Treat every 1 bit in `y` as a bit to toggle and return `x ^ y`; ignore `k`.

## Approach

Use masks and unsigned shifts to implement toggle display channels without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
