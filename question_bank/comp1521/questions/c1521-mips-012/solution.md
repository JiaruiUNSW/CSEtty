# MIPS Buffer Range Function — worked solution

## Idea in one sentence

Translate the loop invariant for mips buffer range function into a leaf MIPS function.

## Exact rule

Return `maximum - minimum`, or 0 for an empty array.

## Approach

Translate the loop invariant for mips buffer range function into a leaf MIPS function. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
