# Count Active Relay Bits — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement count active relay bits without string conversion.

## Exact rule

Return the number of 1 bits in the 32-bit representation of `x`; ignore `y` and `k`.

## Approach

Use masks and unsigned shifts to implement count active relay bits without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
