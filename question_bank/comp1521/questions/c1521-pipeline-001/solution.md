# Pipe-Transferred Child Sum — worked solution

## Idea in one sentence

Compute pipe-transferred child sum in the child and use the pipe as the only result channel back to the parent.
...
Compute pipe-transferred child sum in the child and use the pipe as the only result channel back to the parent. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
