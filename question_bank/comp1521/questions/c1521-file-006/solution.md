# Binary XOR Fingerprint — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute binary xor fingerprint.

## Exact rule

Bitwise-XOR every unsigned byte value, starting from zero.

## Approach

Read the file as bytes and scan once to compute binary xor fingerprint. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
