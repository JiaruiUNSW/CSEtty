# UTF-8 Encoded Width — worked solution

## Idea in one sentence

Classify the scalar range first, then derive utf-8 encoded width from the UTF-8/UTF-16 boundary table.
...
Classify the scalar range first, then derive utf-8 encoded width from the UTF-8/UTF-16 boundary table. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
