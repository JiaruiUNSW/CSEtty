# UTF-8 Byte Checksum — worked solution

## Idea in one sentence

Classify the scalar range first, then derive utf-8 byte checksum from the UTF-8/UTF-16 boundary table.

## Exact rule

Add the unsigned byte values in the canonical UTF-8 encoding; return -1 for a non-scalar.

## Approach

Classify the scalar range first, then derive utf-8 byte checksum from the UTF-8/UTF-16 boundary table. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
