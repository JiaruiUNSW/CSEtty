# Longest Identical Byte Run — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute longest identical byte run.

## Exact rule

Return the longest contiguous run of identical bytes, or 0 for an empty file.

## Approach

Read the file as bytes and scan once to compute longest identical byte run. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
