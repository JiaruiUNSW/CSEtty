...
#include <stdlib.h>
#include <unistd.h>
static long long solve(const unsigned char*a,size_t n){(void)a;(void)n;if(n==0)return 0;long long best=1,run=1;for(size_t i=1;i<n;i++){run=a[i]==a[i-1]?run+1:1;if(run>best)best=run;}return best;}
