# Recursive Build Source Count — worked solution

## Idea in one sentence

Use depth-first traversal and merge each entry into a summary for recursive build source count.

## Exact rule

Count real regular files whose basename ends exactly with the case-sensitive suffix `.c`.

## Approach

Use depth-first traversal and merge each entry into a summary for recursive build source count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
