# Decode a MIPS R-Format Register Checksum — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement decode a mips r-format register checksum without string conversion.

## Exact rule

From `x`, extract rs (bits 25..21), rt (20..16), rd (15..11), and funct (5..0), then return their unsigned sum.

## Approach

Use masks and unsigned shifts to implement decode a mips r-format register checksum without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
