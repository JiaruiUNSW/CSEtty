# Enable Permission Bits — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement enable permission bits without string conversion.

## Exact rule

Treat every 1 bit in `y` as a bit to enable and return `x | y`; ignore `k`.

## Approach

Use masks and unsigned shifts to implement enable permission bits without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
