# First Zero Byte Offset — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute first zero byte offset.

## Exact rule

Return the zero-based offset of the first `0x00` byte, or -1 if none exists.

## Approach

Read the file as bytes and scan once to compute first zero byte offset. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
