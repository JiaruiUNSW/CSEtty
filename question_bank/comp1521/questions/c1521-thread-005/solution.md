# Threaded Weighted Checksum — worked solution

## Idea in one sentence

Partition indices by residue modulo three and merge local values for threaded weighted checksum under a mutex.
...
Partition indices by residue modulo three and merge local values for threaded weighted checksum under a mutex. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
