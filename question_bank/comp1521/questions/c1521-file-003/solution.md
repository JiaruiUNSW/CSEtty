# Binary Zero Byte Count — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute binary zero byte count.
...
Read the file as bytes and scan once to compute binary zero byte count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
