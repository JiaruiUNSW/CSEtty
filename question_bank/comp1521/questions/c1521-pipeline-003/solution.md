# Pipe-Transferred Even Count — worked solution

## Idea in one sentence

Compute pipe-transferred even count in the child and use the pipe as the only result channel back to the parent.

## Exact rule

Count input elements divisible by two, including zero and negative even values.

## Approach

Compute pipe-transferred even count in the child and use the pipe as the only result channel back to the parent. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
