# Rotate a Network Sequence Word — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement rotate a network sequence word without string conversion.

## Exact rule

Let `s = k % 32`; rotate `x` left by `s` bits, wrapping shifted-out bits into the low end.

## Approach

Use masks and unsigned shifts to implement rotate a network sequence word without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
