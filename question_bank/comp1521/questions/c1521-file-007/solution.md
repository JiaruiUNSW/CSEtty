# Printable ASCII Byte Count — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute printable ascii byte count.
...
Read the file as bytes and scan once to compute printable ascii byte count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
