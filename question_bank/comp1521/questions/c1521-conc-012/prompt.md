...
- Write `c1521_conc_012.c`.
- Accept one to eight strings and create one thread per string.
- All workers share a 26-element counter array and one initialized `pthread_mutex_t`.
- For every ASCII letter, convert to lowercase, lock the mutex, increment that letter's counter, and unlock.
- Ignore other bytes.
- Join all workers, print only nonzero counters from `a` to `z` as `LETTER=N`, destroy the mutex, and return 0.
- Any pthread failure returns 1 after the safest possible cleanup.
