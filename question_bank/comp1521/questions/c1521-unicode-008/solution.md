# UTF-8 Boundary Class — worked solution

## Idea in one sentence

Classify the scalar range first, then derive utf-8 boundary class from the UTF-8/UTF-16 boundary table.

## Exact rule

Return 0 for a non-scalar; otherwise return 1, 2, 3, or 4 according to the scalar's canonical UTF-8 width.

## Approach

Classify the scalar range first, then derive utf-8 boundary class from the UTF-8/UTF-16 boundary table. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
