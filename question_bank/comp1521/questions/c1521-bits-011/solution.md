# Swap Byte Nibbles — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement swap byte nibbles without string conversion.

## Exact rule

Within each of the four bytes of `x`, swap its high four bits with its low four bits.

## Approach

Use masks and unsigned shifts to implement swap byte nibbles without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
