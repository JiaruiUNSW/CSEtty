# Unicode Scalar Validity — worked solution

## Idea in one sentence

Classify the scalar range first, then derive unicode scalar validity from the UTF-8/UTF-16 boundary table.

## Exact rule

Return 1 exactly when `cp <= 0x10FFFF` and `cp` is not in the surrogate range `0xD800..0xDFFF`; otherwise return 0.

## Approach

Classify the scalar range first, then derive unicode scalar validity from the UTF-8/UTF-16 boundary table. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
