...
**Exact rule.** Return 1 exactly when `cp <= 0x10FFFF` and `cp` is not in the surrogate range `0xD800..0xDFFF`; otherwise return 0.

Submit `c1521_unicode_001.c`. Your program must not print prompts or explanatory text.
