# Recursive Regular File Count — worked solution

## Idea in one sentence

Use depth-first traversal and merge each entry into a summary for recursive regular file count.

## Exact rule

Count real regular files anywhere below the supplied root; directories and the fixture-only `.keep` marker do not count.

## Approach

Use depth-first traversal and merge each entry into a summary for recursive regular file count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
