# Complete Four-Byte Records — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute complete four-byte records.

## Exact rule

Return the number of complete non-overlapping four-byte records, which is `file_size / 4` using integer division.

## Approach

Read the file as bytes and scan once to compute complete four-byte records. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
