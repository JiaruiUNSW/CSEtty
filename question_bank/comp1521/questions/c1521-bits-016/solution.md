# Reverse a Diagnostic Bit String — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement reverse a diagnostic bit string without string conversion.

## Exact rule

Return the 32-bit word whose bit `31 - i` equals bit `i` of `x` for every `i` from 0 to 31.

## Approach

Use masks and unsigned shifts to implement reverse a diagnostic bit string without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
