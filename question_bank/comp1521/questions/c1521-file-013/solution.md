# Even Byte Count — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute even byte count.

## Exact rule

Count unsigned byte values divisible by two, including `0x00`.

## Approach

Read the file as bytes and scan once to compute even byte count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
