# Position-Weighted Byte Checksum — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute position-weighted byte checksum.

## Exact rule

Return `sum((i + 1) * byte[i]) modulo 2^31`, using unsigned bytes and one-based offsets.

## Approach

Read the file as bytes and scan once to compute position-weighted byte checksum. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
