# Combine Redundant Sensor Masks — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement combine redundant sensor masks without string conversion.

## Exact rule

Return the 32-bit bitwise exclusive OR `x ^ y`; ignore `k`.

## Approach

Use masks and unsigned shifts to implement combine redundant sensor masks without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
