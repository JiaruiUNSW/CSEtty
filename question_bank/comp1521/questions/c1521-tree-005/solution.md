# Recursive Directory Count — worked solution

## Idea in one sentence

Use depth-first traversal and merge each entry into a summary for recursive directory count.

## Exact rule

Count the supplied root and every real directory recursively below it; regular files do not count.

## Approach

Use depth-first traversal and merge each entry into a summary for recursive directory count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
