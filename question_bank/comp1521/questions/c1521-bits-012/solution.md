# Swap Register Halfwords — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement swap register halfwords without string conversion.

## Exact rule

Swap bits 31..16 with bits 15..0 of `x`; ignore `y` and `k`.

## Approach

Use masks and unsigned shifts to implement swap register halfwords without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
