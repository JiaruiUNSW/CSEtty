# POSIX Byte Count — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute posix byte count.

## Exact rule

Return the exact number of bytes in the file; an empty file has size 0.

## Approach

Read the file as bytes and scan once to compute posix byte count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
