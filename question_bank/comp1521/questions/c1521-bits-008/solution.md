# Rotate a Control Word Right — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement rotate a control word right without string conversion.

## Exact rule

Let `s = k % 32`; rotate `x` right by `s` bits, wrapping shifted-out bits into the high end.

## Approach

Use masks and unsigned shifts to implement rotate a control word right without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
