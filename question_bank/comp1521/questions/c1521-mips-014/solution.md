# MIPS Equal Neighbour Count — worked solution

## Idea in one sentence

Translate the loop invariant for mips equal neighbour count into a leaf MIPS function.

## Exact rule

Count adjacent pairs whose values are equal; overlapping pairs count separately.

## Approach

Translate the loop invariant for mips equal neighbour count into a leaf MIPS function. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
