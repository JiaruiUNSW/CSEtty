# Pack Two Sixteen-Bit Counters — worked solution

## Idea in one sentence

Use masks and unsigned shifts to implement pack two sixteen-bit counters without string conversion.

## Exact rule

Place the low 16 bits of `x` in result bits 31..16 and the low 16 bits of `y` in bits 15..0.

## Approach

Use masks and unsigned shifts to implement pack two sixteen-bit counters without string conversion. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
