# Adjacent Byte Transition Count — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute adjacent byte transition count.
...
Read the file as bytes and scan once to compute adjacent byte transition count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
