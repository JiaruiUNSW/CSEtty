# High-Bit Byte Count — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute high-bit byte count.

## Exact rule

Count bytes whose most significant bit is 1, equivalently values from 128 through 255.

## Approach

Read the file as bytes and scan once to compute high-bit byte count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
