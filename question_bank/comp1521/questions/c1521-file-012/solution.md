# Last Newline Offset — worked solution

## Idea in one sentence

Read the file as bytes and scan once to compute last newline offset.

## Exact rule

Return the zero-based offset of the last `0x0A` byte, or -1 if none exists.

## Approach

Read the file as bytes and scan once to compute last newline offset. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
