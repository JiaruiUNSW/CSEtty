# Median Calibration Reading — worked solution

## Idea in one sentence

Compare the three values in a fixed order to compute median calibration reading.

## Exact rule

Return the middle value after ordering the three inputs; repeated values are retained.

## Approach

Compare the three values in a fixed order to compute median calibration reading. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
