# Alternating Buoy Chain — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for alternating buoy chain.

## Exact rule

Compute `node[0] - node[1] + node[2] - ...` in list order.

## Approach

Traverse the chain in order and maintain the minimal state for alternating buoy chain. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
