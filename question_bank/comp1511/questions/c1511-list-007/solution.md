# Recursive Reef Chain Total — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for recursive reef chain total.

## Exact rule

Recursively return `head->value + total(head->next)`, with `total(NULL) = 0`.

## Approach

Traverse the chain in order and maintain the minimal state for recursive reef chain total. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
