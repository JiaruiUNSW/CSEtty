...
Count elements strictly less than zero; zero is not negative.

## Background

The data comes from a cold-chain sensor feed and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_002.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
