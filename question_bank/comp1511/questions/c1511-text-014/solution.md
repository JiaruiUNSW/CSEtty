# First Numeric Field Position — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for first numeric field position.

## Exact rule

Return the zero-based index of the first decimal digit, or -1 when no digit occurs.

## Approach

Scan the line from left to right and retain only the state needed for first numeric field position. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
