# Chain Endpoint Difference — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for chain endpoint difference.

## Exact rule

Return the absolute difference between the first and last node values, or 0 for an empty list.

## Approach

Traverse the chain in order and maintain the minimal state for chain endpoint difference. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
