...
Return `maximum - minimum`, or 0 when the array is empty.

## Background

The data comes from a habitat elevation survey and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_011.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
