...
The data in an electronics bench log arrives as a bounded integer array. A small, auditable metric is needed before the next processing stage.
...
**Exact rule.** Return the maximum length of a contiguous run of equal values, or 0 for empty input.

Submit `c1511_array_014.c`. Your program must not print prompts or explanatory text.
