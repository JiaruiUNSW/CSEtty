# Rising Relay Links — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for rising relay links.

## Exact rule

Count links whose next node has a strictly greater value.

## Approach

Traverse the chain in order and maintain the minimal state for rising relay links. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
