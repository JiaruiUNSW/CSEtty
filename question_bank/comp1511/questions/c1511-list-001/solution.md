# Field Sample Chain Total — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for field sample chain total.

## Exact rule

Return the sum of every node value, with the empty-list result 0.

## Approach

Traverse the chain in order and maintain the minimal state for field sample chain total. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
