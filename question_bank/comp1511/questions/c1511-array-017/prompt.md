...
Count prefixes whose cumulative sum from index 0 is greater than or equal to zero.

## Background

The data comes from a cumulative solvent ledger and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_017.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
