# Peak Signal Nodes — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for peak signal nodes.

## Exact rule

Count non-endpoint nodes whose value is strictly greater than both adjacent node values.

## Approach

Traverse the chain in order and maintain the minimal state for peak signal nodes. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
