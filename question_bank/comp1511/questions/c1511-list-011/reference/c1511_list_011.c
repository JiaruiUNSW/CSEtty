...
static long long solve(const struct node*h){long long r=0;if(!h)return 0;for(;h->next&&h->next->next;h=h->next)if(h->next->value>h->value&&h->next->value>h->next->next->value)r++;return r;}
