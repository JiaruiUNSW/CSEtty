# Recursive Fault Node Count — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for recursive fault node count.

## Exact rule

Recursively count nodes whose value is strictly negative.

## Approach

Traverse the chain in order and maintain the minimal state for recursive fault node count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
