# Shipment Value Range — worked solution

## Idea in one sentence

Store each input row as one struct and compute shipment value range over the resulting record array.

## Exact rule

Return maximum record value minus minimum record value, or 0 for no records.

## Approach

Store each input row as one struct and compute shipment value range over the resulting record array. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
