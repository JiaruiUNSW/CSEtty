# Words in a Dispatch Message — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for words in a dispatch message.

## Exact rule

Count maximal non-whitespace runs; any ASCII whitespace separates words.

## Approach

Scan the line from left to right and retain only the state needed for words in a dispatch message. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
