# Alternating Drone Payload — worked solution

## Idea in one sentence

Scan the array while maintaining exactly the state needed for alternating drone payload.
...
Scan the array while maintaining exactly the state needed for alternating drone payload. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
