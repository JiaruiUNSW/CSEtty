# Uppercase Map Markers — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for uppercase map markers.
...
Scan the line from left to right and retain only the state needed for uppercase map markers. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
