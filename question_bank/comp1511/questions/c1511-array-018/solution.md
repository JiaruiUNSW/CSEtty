# Robot Route Distance — worked solution

## Idea in one sentence

Scan the array while maintaining exactly the state needed for robot route distance.
...
Scan the array while maintaining exactly the state needed for robot route distance. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
