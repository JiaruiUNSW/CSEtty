...
The data in a one-dimensional robot route arrives as a bounded integer array. A small, auditable metric is needed before the next processing stage.
...
**Exact rule.** Sum `abs(a[i] - a[i-1])` over all adjacent pairs.

Submit `c1511_array_018.c`. Your program must not print prompts or explanatory text.
