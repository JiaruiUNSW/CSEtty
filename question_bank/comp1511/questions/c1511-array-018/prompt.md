...
Sum `abs(a[i] - a[i-1])` over all adjacent pairs.

## Background

The data comes from a one-dimensional robot route and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_018.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
