# Bounded Pump Setting — worked solution

## Idea in one sentence

Compare the three values in a fixed order to compute bounded pump setting.

## Exact rule

Treat the first and third inputs as unordered inclusive bounds and clamp the second input between them.

## Approach

Compare the three values in a fixed order to compute bounded pump setting. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
