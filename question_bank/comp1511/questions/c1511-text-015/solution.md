# Normalised Identifier Palindrome — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for normalised identifier palindrome.

## Exact rule

Remove non-alphanumeric characters, fold letters to lowercase, then return 1 iff the result is a palindrome.

## Approach

Scan the line from left to right and retain only the state needed for normalised identifier palindrome. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
