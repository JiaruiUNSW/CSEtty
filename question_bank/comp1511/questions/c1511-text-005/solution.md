# Lowercase Gene Symbols — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for lowercase gene symbols.
...
Scan the line from left to right and retain only the state needed for lowercase gene symbols. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
