# Longest Nondecreasing Record Run — worked solution

## Idea in one sentence

Store each input row as one struct and compute longest nondecreasing record run over the resulting record array.

## Exact rule

Return the longest contiguous nondecreasing run of record values, or 0 for no records.

## Approach

Store each input row as one struct and compute longest nondecreasing record run over the resulting record array. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
