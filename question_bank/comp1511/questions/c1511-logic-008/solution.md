# Closest Reading to Zero — worked solution

## Idea in one sentence

Compare the three values in a fixed order to compute closest reading to zero.

## Exact rule

Return the value with smallest absolute magnitude; break a magnitude tie by choosing the smaller value.

## Approach

Compare the three values in a fixed order to compute closest reading to zero. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
