...
The data in a mountain weather transect arrives as a bounded integer array. A small, auditable metric is needed before the next processing stage.
...
**Exact rule.** Count interior indices `i` where `a[i] > a[i-1]` and `a[i] > a[i+1]`; endpoints never count.

Submit `c1511_array_004.c`. Your program must not print prompts or explanatory text.
