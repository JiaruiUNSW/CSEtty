...
The data in a specimen label batch arrives as a bounded integer array. A small, auditable metric is needed before the next processing stage.
...
**Exact rule.** Count distinct integer values; repeated occurrences contribute only once.

Submit `c1511_array_016.c`. Your program must not print prompts or explanatory text.
