# Even Cargo Nodes — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for even cargo nodes.

## Exact rule

Count nodes containing an even integer, including zero and negative evens.

## Approach

Traverse the chain in order and maintain the minimal state for even cargo nodes. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
