# Grid Value Range — worked solution

## Idea in one sentence

Visit exactly the cells participating in grid value range and maintain one scalar result.

## Exact rule

Return maximum cell minus minimum cell, or 0 for an empty grid.

## Approach

Visit exactly the cells participating in grid value range and maintain one scalar result. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
