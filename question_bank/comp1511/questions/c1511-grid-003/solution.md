# First Highest-Energy Row — worked solution

## Idea in one sentence

Visit exactly the cells participating in first highest-energy row and maintain one scalar result.

## Exact rule

Return the first row index with greatest row sum, or -1 if either dimension is zero.

## Approach

Visit exactly the cells participating in first highest-energy row and maintain one scalar result. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
