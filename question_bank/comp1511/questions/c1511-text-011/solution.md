# Case Transition Counter — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for case transition counter.

## Exact rule

Ignore nonletters, then count transitions between uppercase and lowercase among consecutive remaining letters.

## Approach

Scan the line from left to right and retain only the state needed for case transition counter. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
