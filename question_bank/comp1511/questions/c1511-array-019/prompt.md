...
Compute `sum((i + 1) * a[i])` using one-based position weights.

## Background

The data comes from a numbered archive shelf and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_019.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
