# Archive Position Checksum — worked solution

## Idea in one sentence

Scan the array while maintaining exactly the state needed for archive position checksum.
...
Scan the array while maintaining exactly the state needed for archive position checksum. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
