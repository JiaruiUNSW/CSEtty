# Case-Folded Letter Checksum — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for case-folded letter checksum.

## Exact rule

For each ASCII letter add its case-insensitive alphabet position (`a=1` through `z=26`).

## Approach

Scan the line from left to right and retain only the state needed for case-folded letter checksum. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
