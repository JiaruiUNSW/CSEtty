...
Return the zero-based index of the first maximum, or -1 for an empty array.

## Background

The data comes from an event gate counter and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_012.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
