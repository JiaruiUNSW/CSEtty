...
The data in a flood-monitoring transect arrives as a bounded integer array. A small, auditable metric is needed before the next processing stage.
...
**Exact rule.** Count interior indices `i` where `a[i] < a[i-1]` and `a[i] < a[i+1]`; comparisons are strict.

Submit `c1511_array_005.c`. Your program must not print prompts or explanatory text.
