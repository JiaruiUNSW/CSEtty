...
Count interior indices `i` where `a[i] < a[i-1]` and `a[i] < a[i+1]`; comparisons are strict.

## Background

The data comes from a flood-monitoring transect and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_005.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
