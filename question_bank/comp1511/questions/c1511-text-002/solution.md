# Digits in a Sample Label — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for digits in a sample label.

## Exact rule

Count ASCII decimal characters from `0` through `9`.

## Approach

Scan the line from left to right and retain only the state needed for digits in a sample label. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
