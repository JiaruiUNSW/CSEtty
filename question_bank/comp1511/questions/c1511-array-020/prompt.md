...
Count pairs `i < j` for which `a[i] > a[j]`.

## Background

The data comes from a seed-sorting trial and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_020.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
