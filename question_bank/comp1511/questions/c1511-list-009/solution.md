# Symmetric Beacon Chain — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for symmetric beacon chain.

## Exact rule

Return 1 exactly when the node-value sequence reads the same forwards and backwards; the empty list counts as a palindrome.

## Approach

Traverse the chain in order and maintain the minimal state for symmetric beacon chain. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
