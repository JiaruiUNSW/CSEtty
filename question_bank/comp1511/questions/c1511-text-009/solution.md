# Balanced Console Parentheses — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for balanced console parentheses.

## Exact rule

Return 1 for a balanced parenthesis sequence and 0 otherwise; non-parenthesis characters are ignored.

## Approach

Scan the line from left to right and retain only the state needed for balanced console parentheses. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
