# Repeated Keyboard Strokes — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for repeated keyboard strokes.

## Exact rule

Count adjacent character pairs that contain the same byte.

## Approach

Scan the line from left to right and retain only the state needed for repeated keyboard strokes. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
