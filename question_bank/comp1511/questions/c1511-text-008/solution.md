# Maximum Bracket Nesting — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for maximum bracket nesting.
...
Scan the line from left to right and retain only the state needed for maximum bracket nesting. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
