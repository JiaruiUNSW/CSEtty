# Longest Nondecreasing Trail — worked solution

## Idea in one sentence

Scan the array while maintaining exactly the state needed for longest nondecreasing trail.

## Exact rule

Return the maximum length of a contiguous nondecreasing run (`next >= current`).

## Approach

Scan the array while maintaining exactly the state needed for longest nondecreasing trail. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
