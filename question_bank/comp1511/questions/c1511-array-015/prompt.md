...
The data in a hiking trail profile arrives as a bounded integer array. A small, auditable metric is needed before the next processing stage.
...
**Exact rule.** Return the maximum length of a contiguous nondecreasing run (`next >= current`).

Submit `c1511_array_015.c`. Your program must not print prompts or explanatory text.
