# Longest Identical Wagon Run — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for longest identical wagon run.

## Exact rule

Return the longest contiguous run of equal node values, or 0 for an empty list.

## Approach

Traverse the chain in order and maintain the minimal state for longest identical wagon run. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
