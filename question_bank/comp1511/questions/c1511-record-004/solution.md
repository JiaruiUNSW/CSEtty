# Equal-Reading Record Pairs — worked solution

## Idea in one sentence

Store each input row as one struct and compute equal-reading record pairs over the resulting record array.
...
Store each input row as one struct and compute equal-reading record pairs over the resulting record array. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
