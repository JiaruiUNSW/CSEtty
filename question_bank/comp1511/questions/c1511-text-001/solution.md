# Vowels in a Radio Transcript — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for vowels in a radio transcript.
...
Scan the line from left to right and retain only the state needed for vowels in a radio transcript. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
