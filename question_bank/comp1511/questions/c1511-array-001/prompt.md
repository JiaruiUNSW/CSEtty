...
Sum only values strictly greater than zero; return 0 for an empty array.

## Background

The data comes from a community battery log and is stored as a bounded integer array. The array order is significant whenever the task refers to positions or neighbours.
...
Submit `c1511_array_001.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
