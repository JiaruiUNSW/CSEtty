# Longest Emergency Call Word — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for longest emergency call word.
...
Scan the line from left to right and retain only the state needed for longest emergency call word. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
