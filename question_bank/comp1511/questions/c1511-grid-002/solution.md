# Survey Main Diagonal — worked solution

## Idea in one sentence

Visit exactly the cells participating in survey main diagonal and maintain one scalar result.

## Exact rule

Sum cells `(i, i)` for `0 <= i < min(rows, columns)`.

## Approach

Visit exactly the cells participating in survey main diagonal and maintain one scalar result. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
