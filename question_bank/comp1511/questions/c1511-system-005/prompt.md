...
An operator maintains festival token accounts through a line-oriented command console. Entries appear dynamically and must remain correct across updates and removals.
...
Submit `c1511_system_005.c` only. Your program must not print prompts, labels, or explanatory text unless the required output format explicitly includes them.
