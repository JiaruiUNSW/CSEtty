# Distinct Channel Count — worked solution

## Idea in one sentence

Compare the three values in a fixed order to compute distinct channel count.

## Exact rule

Return 1, 2, or 3 according to the number of distinct input values.

## Approach

Compare the three values in a fixed order to compute distinct channel count. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
