# First Maximum Checkpoint — worked solution

## Idea in one sentence

Traverse the chain in order and maintain the minimal state for first maximum checkpoint.

## Exact rule

Return the zero-based position of the first maximum node, or -1 for an empty list.

## Approach

Traverse the chain in order and maintain the minimal state for first maximum checkpoint. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
