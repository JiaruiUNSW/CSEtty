# Character Runs in a Barcode Note — worked solution

## Idea in one sentence

Scan the line from left to right and retain only the state needed for character runs in a barcode note.

## Exact rule

Count maximal contiguous runs of identical characters; the empty line has 0 runs.

## Approach

Scan the line from left to right and retain only the state needed for character runs in a barcode note. Keep the input, state update, and output-formatting phases separate so each can be checked independently.
